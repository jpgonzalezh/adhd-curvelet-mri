# ============================================================
# LOSO EXPERIMENTO 3 - LEAVE-ONE-SITE-OUT CROSS-DATASET
# Objetivo: robustez al sitio (Opción A)
# Training: ADHD-200 sin un sitio → Test: WMR-ADHD completo (fijo)
# + Baseline con todos los sitios incluidos
# Nested CV dentro del training
...
configs = [
    {
        ...
        'output': f'{BASE_PATH}/comparison_sample/results_loso_exp3',
        'name':   '201'
    },
    {
        'csv':    f'{BASE_PATH}/649_subjects/curvelet_data_adhd_649_loso.csv',
        'output': f'{BASE_PATH}/649_subjects/results_loso_exp3',
        'name':   '649'
    },
    {
        'csv':    f'{BASE_PATH}/875_subjects/curvelet_data_adhd_875_loso.csv',
        'output': f'{BASE_PATH}/875_subjects/results_loso_exp3',
        'name':   '875'
    }
]
...
# ── 4. FUNCIÓN POR SITIO EXCLUIDO (o baseline) ───────────────
def run_loso_site(site_excluded, data_train_full, data_test,
                  feature_cols, regions_list, parameters,
                  RANDOM_SEED, N_BOOTSTRAP, N_PERMUT):

    label = site_excluded if site_excluded else 'None (baseline)'
    print(f"    → Sitio excluido del training: {label}")

    # Si site_excluded es None → baseline con todos los sitios
    if site_excluded is not None:
        data_train = data_train_full[
            data_train_full['site_id'] != site_excluded].copy()
    else:
    ...
    n_train = len(data_train) // data_train['region_index'].nunique()
    n_test  = len(data_test)  // data_test['region_index'].nunique()
    print(f"    Training: {n_train} sujetos | Test WMR: {n_test} sujetos")

    valid_regions = []
    for region in regions_list:
        rt  = data_train[data_train['region_index'] == region]
        rte = data_test[data_test['region_index'] == region]
        if rt['label_binary'].nunique() == 2 and len(rte) > 0:
    ...
    for param in parameters:
        ...
        for region in valid_regions:
            rt = data_train[data_train['region_index'] == region].reset_index(drop=True)
            features_dict_train[(region, param)] = rt[feature_cols].values[:, seq]
            labels_dict_train[(region, param)]   = rt['label_binary'].values

            rte = data_test[data_test['region_index'] == region].reset_index(drop=True)
...
# ── 5. CARGAR WMR-ADHD UNA SOLA VEZ ──────────────────────────
print("Cargando WMR-ADHD...")
feature_cols_global = [f'curv_{i}' for i in range(1, 244)]
data_wmr = pd.read_csv(CSV_WMR, header=None)
data_wmr.columns = ['region_index', 'dx_group'] + feature_cols_global
data_wmr['label_binary'] = data_wmr['dx_group'].apply(
    lambda x: 0 if x == 0 else 1)
print(f"WMR-ADHD: {data_wmr['region_index'].nunique()} regiones")
print(f"Distribución: {data_wmr['label_binary'].value_counts().to_dict()}")

# ── 6. FUNCIÓN PRINCIPAL POR CONJUNTO ────────────────────────
def run_experiment(config, SITES_LOSO, data_wmr):
    ...
    print(f"\n{'='*60}")
    print(f"LOSO EXP 3 CORREGIDO — TRAINING: {name} → TEST: WMR-ADHD")
    print(f"Objetivo: robustez al sitio (Opción A)")
    print(f"{'='*60}")
    start = time.time()
    ...
    data_train_full = pd.read_csv(csv_path, header=None, low_memory=False)
    data_train_full.columns = ['region_index', 'site_id', 'dx_group'] + feature_cols
    data_train_full['label_binary'] = data_train_full['dx_group'].apply(
        lambda x: 0 if x == 0 else 1)
    data_train_full = data_train_full[data_train_full['site_id'].isin(SITES_LOSO)]

    regions_train  = set(data_train_full['region_index'].unique())
    regions_wmr    = set(data_wmr['region_index'].unique())
    regions_common = sorted(regions_train & regions_wmr)

    data_train_full = data_train_full[
        data_train_full['region_index'].isin(regions_common)]
    data_test = data_wmr[
        data_wmr['region_index'].isin(regions_common)].copy()

    parameters      = ['alpha', 'beta', 'mu']
    sites_available = [s for s in SITES_LOSO
                       if s in data_train_full['site_id'].unique()]

    print(f"Regiones comunes (training ∩ WMR): {len(regions_common)}")
    print(f"Sitios disponibles: {sites_available}")

    # ── BASELINE + LOSO ───────────────────────────────────────
    # None = baseline (todos los sitios), luego cada sitio excluido
    sites_to_run = [None] + sites_available

    results = Parallel(n_jobs=N_JOBS, verbose=5)(
        delayed(run_loso_site)(
            site_excl, data_train_full, data_test,
            feature_cols, regions_common, parameters,
            RANDOM_SEED, N_BOOTSTRAP, N_PERMUT
        )
        for site_excl in sites_to_run
    )
    ...
    # ── RANK STABILITY (solo folds LOSO, no baseline) ─────────
    df_loso_only = df_results[df_results['Site_excluded'] != 'None (baseline)']
    region_counts = Counter(zip(
        df_loso_only['Best_region'], df_loso_only['Best_parameter']))
    df_rank = pd.DataFrame([
        {'Region': r, 'Parameter': p,
         'Selection_count': c,
         'Selection_freq_%': round(c / len(sites_available) * 100, 1)}
        for (r, p), c in region_counts.most_common()
    ])

    # ── GUARDAR ───────────────────────────────────────────────
    df_results.to_csv(f'{output_dir}/loso_results_{name}.csv', index=False)
    df_rank.to_csv(f'{output_dir}/loso_rank_stability_{name}.csv', index=False)

    df_confmat = df_results[[
        'Site_excluded', 'N_train', 'N_test',
    ...
    return set(zip(df_loso_only['Best_region'], df_loso_only['Best_parameter']))
...
if __name__ == '__main__':
    ...
    for config in configs:
        top_regions = run_experiment(config, SITES_LOSO, data_wmr)
        if top_regions is not None:
            rank_stability_all[config['name']] = top_regions

    # ── JACCARD ───────────────────────────────────────────────
    print(f"\n{'='*60}")
    print("JACCARD OVERLAP LOSO EXP 3 — ENTRE CONJUNTOS")
    print(f"{'='*60}")
    ...
    for i in range(len(names)):
        for j in range(i+1, len(names)):
            ...
            jaccard_rows.append({'Set_A': names[i], 'Set_B': names[j],
    ...
    for config in configs:
        df_jaccard.to_csv(
            f'{config["output"]}/loso_jaccard_exp3.csv', index=False)
