# ============================================================
# SENSITIVITY ANALYSIS — SCANNER CONFOUND (excluding NeuroIMAGE)
# NeuroIMAGE is the only 1.5T site; all others are 3T (Table 2)
# Subsample: 875 - NeuroIMAGE (72) = 803 (514 TDC + 289 ADHD)
# Nested CV (inner=10, outer=10) + Permutation test + FDR
# Mismo pipeline que exp2_5inner.py
# ============================================================
...
OUTPUT_DIR = f'{BASE_PATH}/875_subjects/results_exp2_no_neuroimage'
...
if __name__ == '__main__':
    ...
    data = pd.read_csv(CURV_PATH, usecols=['region_index','subject_id','site_id','dx_group'] + feature_cols)
    data = data.dropna(subset=['subject_id'])
    data['subject_id'] = data['subject_id'].astype(str).str.strip().str.split('.').str[0]

    # Excluir NeuroIMAGE (único sitio 1.5T)
    data = data[data['site_id'] != 'NeuroIMAGE']
    data['label_binary'] = data['dx_group'].apply(lambda x: 0 if x==0 else 1)

    print(f"Sujetos únicos (sin NeuroIMAGE): {data['subject_id'].nunique()}")
    print(f"Sitios incluidos: {sorted(data['site_id'].unique())}")
    ...
    print(f"Sujetos totales (region-aligned): {n_subjects}")
    ...
    df_outer.to_csv(f'{OUTPUT_DIR}/nested_cv_results_no_neuroimage.csv', index=False)
    df_rank.to_csv(f'{OUTPUT_DIR}/rank_stability_no_neuroimage.csv', index=False)
