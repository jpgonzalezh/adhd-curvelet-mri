# ============================================================
# SENSITIVITY ANALYSIS — AGE STRATIFIED (separado)
# Corre Exp 2 completo (nested CV) por separado en:
#   - Children (6 <= age < 12): N=499 (312 TDC + 187 ADHD)
#   - Adolescents (12 <= age < 18): N=315 (191 TDC + 124 ADHD)
# Sujetos >=18 años (N=61) excluidos
# Age groups based on Fonov et al. (2011) NeuroImage pediatric atlas range (4.5-18.5y)
...
configs = [
    {'output': f'{BASE_PATH}/875_subjects/results_exp2_children_only',    'name': 'children',    'age_min': 6,  'age_max': 12},
    {'output': f'{BASE_PATH}/875_subjects/results_exp2_adolescents_only', 'name': 'adolescents', 'age_min': 12, 'age_max': 18}
]

# ── 2. OBTENER IDS POR GRUPO DE EDAD ──────────────────────────
def get_ids_by_age(age_min, age_max):
    ...
    pheno['Age_clean']  = pd.to_numeric(pheno['Age'], errors='coerce')
    ...
    merged = curv_ids.merge(pheno[['subject_id','Age_clean','label']], on='subject_id', how='left')
    merged = merged.dropna(subset=['Age_clean','label'])
    sub = merged[(merged['Age_clean']>=age_min) & (merged['Age_clean']<age_max)]

    n_tdc = (sub['label']=='TDC').sum()
    n_adhd = (sub['label']=='ADHD').sum()
    print(f"  Age [{age_min},{age_max}): N={len(sub)}, TDC={n_tdc}, ADHD={n_adhd}")
...
# ── 6. FUNCIÓN PRINCIPAL POR GRUPO DE EDAD ────────────────────
def run_experiment(config):
    ...
    print(f"\n{'='*60}")
    print(f"CORRIENDO EXP 2 ESTRATIFICADO — {name.upper()} [{age_min},{age_max})")
    print(f"{'='*60}")
    start = time.time()
    os.makedirs(output_dir, exist_ok=True)

    print("Obteniendo IDs...")
    selected_ids = get_ids_by_age(age_min, age_max)
    ...
    region_counts2 = Counter(zip(df_outer['Best_region'], df_outer['Best_parameter']))
    df_rank = pd.DataFrame([
        {'Region': r, 'Parameter': p, 'Selection_count': c,
         'Selection_freq_%': round(c/10*100, 1)}
        for (r,p), c in region_counts2.most_common()
    ])
